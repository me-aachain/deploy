...
function create_producers() {
    ...
    do
        ...
        #cleos transfer eosio $name "90909090.9090 AAA"          ;eval $error_d_m
    done

    for ((i = $start; i <= $end ; i++))
    do
        local name=$(eval echo \${producer${i}_name})
        cleos push action eosio.stable transfer '[ "eosio", "'$name'", "100000.0000 ASDT", "m" ]' -p eosio@active          ;eval $error_d_m
    done
